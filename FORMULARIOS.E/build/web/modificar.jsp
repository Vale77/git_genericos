...
<html>
    ...
    <body> 
        ...
        <%logger.fatal("Show FATAL message"); %>f
        <%
            try {
        ...
        <form action="modificado.jsp" method="POST">
            <div class="container">
                <%                request.setCharacterEncoding("UTF-8");
                    ...
                    out.println("<input type=\"text\" class=\"col-md-6 panel panel-info panel-heading\" name=\"nombreF\" value=" + "\"" + listaF.getFirst().getNombre_formulario() + "\"/>");
                    out.println("</div>");

                    for (int i = 0; i < listaG.size(); i++) {
                        out.println("<div class=\"row \">");
                        out.println("<div class=\"col-md-3\"></div>");
                        out.println("<input class=\"col-md-6 panel panel-info panel-heading\" name=\"nomG" + i + "\"  value=" + "\"" + listaG.get(i).getNombre_grupo() + "\"/>");
                        out.println("<div class=\"col-md-3\"></div>");
                        out.println("</div>");
                        ...
                        for (int j = 0; j < listaP.size(); j++) {
                            if (listaP.get(j).getCodigo_grupo() == listaG.get(i).getCodigo_grupo()) {
                                out.println("<div class=\"row\">");
                                out.println("<div class=\"col-md-3\"></div>");
                                out.println("<input class=\"col-md-6 panel panel-info panel-heading\" name=\"nomP" + i + "\" value=" + "\"" + listaP.get(j).getLabel_pregunta() + "\"/></br>");
                                ...
                                if (listaP.get(j).getCodigo_tipo_pregunta() == 2) {
                                    ...
                                    for (int k = 0; k < listaV.size(); k++) {
                                        if (listaV.get(k).getCodigo_Preguntas() == listaP.get(j).getCodigo_preguntas()) {
                                            out.println("<div class=\"row\">");
                                            out.println("<div class=\"col-md-3\"></div>");
                                            out.println("<div class=\"col-md-3\"><label><input type=\"text\" name=\"Seleccion" + k + "\" value=" + "\"" + listaV.get(k).getValores() + "\"/></input></label></div>");
                                            out.println("<div class=\"col-md-3\"></div>");
                                            out.println("<div class=\"col-md-3\"></div>");
                                            out.println("</div>");
                                        }
                                ...
                                if (listaP.get(j).getCodigo_tipo_pregunta() == 3) {
                                    ...
                                    for (int k = 0; k < listaV.size(); k++) {
                                        if (listaV.get(k).getCodigo_Preguntas() == listaP.get(j).getCodigo_preguntas()) {
                                            out.println("<div class=\"row\">");
                                            out.println("<div class=\"col-md-3\"></div>");
                                            out.println("<div class=\"col-md-3\"><input type=\"text\" name=\"seleccion" + k + "\" value=" + "\"" + listaV.get(k).getValores() + "\"/></input></div>");
                                            out.println("<div class=\"col-md-3\"></div>");
                                            out.println("<div class=\"col-md-3\"></div>");
                                            out.println("</div>");
                                        }
                    ...
                    out.println("<div class=\"row\">");
                    out.println("<div class=\"col-md-3\"></div>");
                    out.println("<div class=\"col-md-6\"><center><button class=\"btn btn-default\" type=\"text\" name=\"Submit\" value='" + Cod + "'>Modificar</button></center></div>");
                    out.println("<div class=\"col-md-3\"></div>");
                    out.println("</div>");
                %>
            </div>
